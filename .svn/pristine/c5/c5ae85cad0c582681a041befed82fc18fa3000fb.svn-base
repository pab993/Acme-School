...
<display:table name="folders" pagesize="5" class="displaytag" requestURI="${requestURI}" id="row">
	...
	<spring:message code="messageFolder.description" var="headerTag" />
	...
	<display:column title="${headerTag}">
		<jstl:if test="${!row.isSystem}">
			<acme:delete textConfi1="messageFolder.confirm.delete" url="folder/actor/delete.do?folderId=${row.id}" text="messageFolder.delete" textConfi2="messageFolder.confirm.delete"/>
		</jstl:if>
	</display:column>	
</display:table>
