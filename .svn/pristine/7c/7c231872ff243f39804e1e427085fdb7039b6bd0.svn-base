...
public interface FolderRepository extends JpaRepository<Folder, Integer> {
	...
	@Query("select f from Folder f where f.actor.id=?1 and f.name=?2 and f.isSystem=true")
	Folder findActorAndFolder(int idActor, String nameFolder);

	@Query("select f from Folder f where f.actor.id=?1 and f.name=?2 and f.isSystem=?3")
	Folder findActorAndFolderAndIsSystem(int idActor, String nameFolder, Boolean isSystem);
