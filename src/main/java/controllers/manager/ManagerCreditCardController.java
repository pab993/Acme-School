...
public class ManagerCreditCardController extends AbstractController {
	...
	public ModelAndView list(@RequestParam(required = false) String messageSuccess, @RequestParam(required = false) String messageError) {
		...
		result.addObject("listCreditCard", ((Manager) this.actorService.findByPrincipal()).getCreditCards());
	...
	public ModelAndView add() {
		...
		ModelAndView result;
		result = this.createEditModelAndViewCreate(this.creditCardService.create(), null, null);
	...
	public ModelAndView add(@Valid CreditCard creditCard, final BindingResult binding) {
		ModelAndView result;
		this.creditCardService.recontruct(creditCard, binding);
		try {
	...
	public ModelAndView deleted(@RequestParam final int id) {
		...
		ModelAndView result;
		try {
			CreditCard creditCard = this.creditCardService.findOne(id);
